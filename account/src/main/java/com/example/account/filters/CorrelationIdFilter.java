...
/**
 * 把 Gateway 傳下來的 correlation-id 放進 MDC，讓這次請求的每一行 log 都自動帶上它。
 * MDC = log 框架的「當前執行緒變數袋」。放進去之後，application.yaml 的 logging.pattern.console 用 %X{correlationId} 就撈得到，不必每行 log 手動傳。
 */
...
public class CorrelationIdFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        ...
        // 1. 從請求標頭撈 Gateway 傳下來的 ID
        String id = request.getHeader(CORRELATION_ID);

        // 2. 放進 MDC。沒帶就自己生一個 —— 直接打 :8080 繞過 Gateway 測試時也追得到
